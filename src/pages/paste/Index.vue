...
function fileChange(event) {
  event.target.files.forEach((item) => selectedFileList.value.push(item));
}
    ...
    <div class="py-[30px] px-[100px]">
      <div class="flex justify-between mb-[10px]">
        ...
        <button
          ...
          class="rounded-none px-[50px] py-[10px]text-white"
          @click="submit"
        >
          提交
        </button>
      ...
      <label
        for="selectFile"
        :style="{ backgroundColor: themeStore.primary }"
        class="rounded-none w-full block text-center cursor-pointer py-[10px] mt-[10px]text-white"
        >选择附件</label
      >
      <input
        id="selectFile"
