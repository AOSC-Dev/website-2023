...
(async () => {
  const [res, err] = await requestGetJson('/gallery.yml');
  if (res) {
...
<template>
  <div>
    ...
    >
        ...
        >
          <div class="flex justify-center w-[100%] h-[100%]">
            <img
              ...
              :src="`https://aosc.io/assets/i/gallery/${photo.file}`"
              :alt="photo.desc"
            />
          </div>
        </el-carousel-item>
      ...
      <!-- 关闭按钮 -->
      <div
        class="fixed top-[20px] right-[20px] p-[2px] rounded-full cursor-pointer z-[999]"
